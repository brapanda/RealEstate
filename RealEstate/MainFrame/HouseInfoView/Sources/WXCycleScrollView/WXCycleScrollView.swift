...
//            self.resetTimer()
    ...
    public convenience init(frame: CGRect, imageNames: Array<String>) {
        ...
        for subview in self.scrollView.subviews {
            if subview.isMemberOfClass(UIImageView) {
                let imageView = subview as! UIImageView
                let imageData = UIImage.init(named: imageNames[index])
...
//        self.resetTimer()
    ...
    public func SetFrame(offY: CGFloat){
        self.scrollView.frame.size.height = -offY
        self.pageControl.frame.origin.y = -offY - 28
        self.waveView.frame.origin.y = -offY - 9
    }
