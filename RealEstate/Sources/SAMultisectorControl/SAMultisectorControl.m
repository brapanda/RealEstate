...
- (BOOL) continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    NSLog(@"startValue : %g,endValue : %g",trackingSector.startValue,trackingSector.endValue);
    if(trackingSectorStartMarker){
        ...
        if(newValue >= trackingSector.endValue){
            ...
            trackingSector.startValue = trackingSector.endValue - 0.1;
            [self valueChangedNotification];
            [self setNeedsDisplay];
            return YES;
        }
        ...
        if(newValue <= trackingSector.startValue){
            trackingSector.endValue = trackingSector.startValue + 0.1;
            [self valueChangedNotification];
            [self setNeedsDisplay];
            return YES;
        }
...
- (void)drawSector:(SAMultisectorSector *)sector atPosition:(NSUInteger)position{
    ...
    UIColor *endCircleColor = self.endColor;
